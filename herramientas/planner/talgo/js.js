...
//Limpiar campos
  ...
  var numero_autorizador_valor=buscar_Match(/(?<=\D)\d{4}(?=\s{0,}\-\s{0,})/,valor_Textoarea)
  var numero_autorizador_result="RM*ACECRM/N EMPLEADO PETICIONARIO-"+numero_autorizador_valor;
  array_hecho.push(numero_autorizador_result)
    
   // TERCER RM -segunda parte 
   var nombre_autorizador_valor=buscar_Match(/(?<=\D\d{4}\s{0,}\-\s{0,}\s).*/,valor_Textoarea)
   var nombre_autorizador_result="RM*ACECRM/NOMBRE AUTORIZADOR-"+nombre_autorizador_valor; 
